...
def train():
    ...
    for itr in range(start_itr + 1, MAX_TRAIN_ITERS + 1):
        ...
        success_rate = (np.sum([1 for r in ep_returns if r > 0]) / len(ep_returns)) * 100 if ep_returns else 0.0
        print(f"[Itr {itr}] Success Rate (approx): {success_rate:.2f}% | Mean Return: {mean_ret:.2f}")
